...
def warp_udp_probe_scan(candidates: List[str], attempts: int = 3, timeout_ms: int = 1000, concurrency: int = 200) -> List[Dict]:
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=concurrency) as executor:
        future_to_endpoint = {executor.submit(warp_udp_probe_endpoint, ep, attempts, timeout_ms): ep for ep in candidates}
        for future in concurrent.futures.as_completed(future_to_endpoint):
            result = future.result()
            if result["recv"] > 0:  # Only include endpoints that responded
                results.append(result)
    # Sort by average response time
    results.sort(key=lambda x: x["avg_ms"] or float('inf'))
    return results
